...
产品来源：二测 PRD「放水调控优化功能（每日/次日登录）」——完整字段与优先级见
`references/sources/feishu-二测优化-机台事件优化需求.md` §2.1。

`PointsRaceGiftReplace`：登录后若积分事件已开/即将开，按
`(group_unique_id, 替换开启时间, stage范围, rate范围)` 匹配，
取 B=`WindowSpinCount`、C=`GiftCount`，在 1..B 中随机抽 C 个 spin 序号留存；
接下来 B 次事件里，列表标注的次数强制走 `GiftReplaceGroupID`。
**优先级**：非新手期期间，除卡点强行替换外本替换最高。
若窗口未结束前 `OpenHour`（替换开启时间）/`Stage`/`Rate` 任一条件不再满足，则提前终止本次赠送。

### 6.1 首日卡档进度 ≥50%（仅新手首关，二测新增）

配表示意：`group_id` + `stage` + `rate`（例：2001 / 4 / 100）。
新手解锁积分事件后维护「新手 spin 开关」：开关=1 时直接出结果；开关=0 且本次结果为积分事件时，
读 `group_id/stage_after/rate_after`，若与表匹配且进度 ≥ 表内 stage/rate，则开关置 1。
目标体感：首日卡档进度不低于约 50%。流程细节与待确认线上表名见二测 PRD §2.2。
...
**蓄水项（二测方案，优先按此验证）**：按**付费 id** 查新表，取「折算后水池蓄水量」写入水池。
爬塔等解锁型付费不给体力，旧「只按直获体力比例蓄水」会导致大部分付费不生效——新表把**有/无体力付费统一按 id 管理**。
金额列可省略，也可配系数再折算（产品口径：看后端实现方便）。PRD 原文见二测 §2.3。

**旧公式残留**（若线上仍未见 pay_id 表）：
`该次付费能获得的纯体力对应美元价值 × 付费金额对应系数`——系数表历史上未找到；
若观测到仍只对直购体力蓄水，记为「二测按 id 蓄水未落地」，不要只报「系数缺配」。
